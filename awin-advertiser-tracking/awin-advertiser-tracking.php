...
define('AWIN_ADVERTISER_TRACKING_VERSION', '1.2.1');
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))) || (is_multisite() && isset(get_site_option('active_sitewide_plugins')['woocommerce/woocommerce.php']))) {
    ...
    {
        // Check if the product feed generation button was clicked
        if (isset($_POST['awin_generate_product_feed'])) {
            generate_product_feed();                                    // Call your product feed generation function
            add_action('admin_notices', 'awin_feed_generation_notice'); // Add a notice to inform the user
        }
    }
